...
function edgeCollision(bodies){
	for(let i=0;i<bodies.length;i++){
		let obj = bodies[i];
		let energy_lost = obj.energy_lost;
		if(obj.pos.x < obj.size){
			obj.vel.x = Math.abs(obj.vel.x) * energy_lost;
			obj.pos.x = obj.size;
		}else if(obj.pos.x > (cw - obj.size)){
			obj.vel.x = -Math.abs(obj.vel.x) * energy_lost;
			obj.pos.x = cw - obj.size;
		}		
		if(obj.pos.y < obj.size){
			obj.vel.y = Math.abs(obj.vel.y) * energy_lost;
			obj.pos.y = obj.size;
		}else if(obj.pos.y > (ch - obj.size)){
			obj.vel.y = -Math.abs(obj.vel.y) * energy_lost;
			obj.pos.y = ch - obj.size;
		}
...
function distSq(xa,ya,xb,yb){return ((xb-xa)**2)+((yb-ya)**2)};
