...
function draw() {
	...
	for (let junk of junks.junks_arr){
		...
		junks_canvas.ctx.line(0,posY1,sw,posY1,{
			color:"blue"
		});
		junks_canvas.ctx.line(0,junk.pos.y,sw,junk.pos.y,{
			color:"yellow"
		})
		junks_canvas.ctx.line(0,posY2,sw,posY2,{
			color:"violet"
		})

		if (junk.pos.y >= posY1 && junk.pos.y <= posY2) {
			junks_canvas.ctx.box(junk.hitbox.x, junk.hitbox.y, junk.hitbox.w, junk.hitbox.h, {
				color: "green",
				fill: "transparent"
			});
			for (let player_box of player.hitbox) {
				player_canvas.ctx.box(player_box.x, player_box.y, player_box.w, player_box.h, {
					color: "red",
					fill: "transparent"
				});
